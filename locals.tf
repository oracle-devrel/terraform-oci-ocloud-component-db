...
locals {
    ...
    db_vcn_id = var.use_existing_subnet ? var.vcn_id : data.terraform_remote_state.external_stack_remote_state.outputs.service_segment_vcn_id
    # Set the databases target compartment. In case the database is placed in an existing subnet use the input value
    db_compartment_id = var.use_existing_subnet ? var.db_compartment_id : data.terraform_remote_state.external_stack_remote_state.outputs.db_compartment_id
    # Set the target subnet
    db_subnet_id = var.use_existing_subnet ? var.subnet_id : module.db_domain[0].subnet_id
    ...
    # database nodes in the same subnet may not use the same hostname prefix. Adding the project and environment label makes it unique to some extent
    db_system_hostname = format("%s%s%s",var.db_system_hostname,local.project,local.environment)
    
    # By default we use the Bastion Service that is instanciated through the landing zone.
    db_bastion_id = data.terraform_remote_state.external_stack_remote_state.outputs.app_domain_bastion_id
    ...
    project = var.overwrite_project ? var.project : split("_", data.oci_identity_compartment.init_name.name)[1]
    environment = var.overwrite_project ? var.environment : split("_", data.oci_identity_compartment.init_name.name)[2]
